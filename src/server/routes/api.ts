...
const buildDir = async (dir: string): Promise<ReadDirectoryResponse> => {
  ...
  try {
    files = await fs.promises.readdir(dir).then(
      (files) => files.filter((file) => !file.includes('hidden'))
    )
  } catch (err) {
    result.error = true
    return result
...
router.post('/delete', async (req, res) => {
  ...
  if (folders.length > 0) {
    ...
    result.folderResults = await Promise.all(
      folders.map((folder) => fs.promises.rm(folder, { recursive: true })
        .then(() => ({
          error: false, folder: (folder.split('/').pop() as string), message: 'Deleted successfully.'
        }),
        (reason) => ({
          error: true, folder: (folder.split('/').pop() as string), message: reason
        })))
    )
  }
